...
############################################################################################
#
#  App file — Fixed version with doubleScroll working on Reactable
#
#############################################################################################
...
ui <- tagList(
  tags$head(
    ...
    # --- JS initialization for Reactable ---
    tags$script(HTML("
      $(document).ready(function() {
        // Detect when the feature filter Reactable is rendered
        $(document).on('shiny:value', \"[id$='feat_filtered_table']\", function(e) {
          var $out = $(this);

          setTimeout(function() {
            // Locate the inner reactable container
            var $reactable = $out.find('.reactable').first();
            if ($reactable.length === 0) return;

            // Avoid duplicate wrappers
            if ($reactable.parent().attr('id') !== 'scrolldiv') {
              $reactable.wrap('<div id=\"scrolldiv\"></div>');
            }

            // Apply DoubleScroll plugin
            $('#scrolldiv').doubleScroll({
              contentElement: $('#scrolldiv').find('table'),
              scrollCss: {
                'overflow-x': 'scroll',
                'overflow-y': 'hidden'
              },
            ...
            // Trigger resize event to sync scrollbar
